...
DxfParser.prototype._parse = function(dxfString) {
	...
	var parseLayers = function() {
		...
		while(!groupIs(0, 'ENDTAB')) {
			...
			switch(curr.code) {
				 ...
				 * 32 = If both this bit and bit 16 are set, 
				 *    the externally dependent xref has been successfully resolved 
				 * 64 = If set, the table entry was referenced by at least one 
	...
	var parseMTEXT = function() {
		...
		while(curr !== 'EOF') {
			...
			switch(curr.code) {
				 ...
				 * A group code 50 (rotation angle in radians) passed as DXF 
				 *   input is converted to the equivalent direction vector 
				 *   (if both a code 50 and codes 11, 21, 31 are passed, the last one wins). 
				 *   This is provided as a convenience for conversions from text objects
				 */
					entity.xAxisDirection = parsePoint();
					break;
                case 40:
                ...
                case 41:
                    ...
                    curr = scanner.next();
                    break;
				case 50: //Rotation angle in radians
	...
	/**
	 * Used to parse a ellipse entity
	 * @return {Object} the entity parsed
	 */
		...
		while(curr !== 'EOF') {
			...
			switch(curr.code) {
				case 10: // X coordinate of point
					entity.center = parsePoint();
					break;
				case 11: // Endpoint of major axis, relative to the center 
					entity.endPoint = parsePoint();
					break;
				case 40: // Ratio of minor axis to major axis
					entity.ratio = curr.value;
					curr = scanner.next();
					break;
				case 41: // start angle 
